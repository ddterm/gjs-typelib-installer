...
async function findPackageKitInstallCommand(cancellable = null) {
    ...
    if (pkgcli) {
        ...
        try {
            ...
            const backendInfo = JSON.parse(stdout);
            let roles = backendInfo?.roles;
            ...
            if (roles?.includes('install-packages')) {
                ...
                log(`PackageKit backend can't install packages: ${JSON.stringify(stdout)}`);

                return null;  // No need to try pkcon
            }
        } catch (ex) {
            ...
            logError(ex, `${shellJoin(argv)} doesn't seem to work`);
        }
    }
    ...
    if (pkcon) {
        ...
        try {
            ...
            if (stdout.startsWith('Name:')) {
                ...
                throw new Error(`Unexpected output: ${JSON.stringify(stdout)}`);
            }
        } catch (ex) {
            if (ex instanceof GLib.Error &&
                ex.matches(Gio.io_error_quark(), Gio.IOErrorEnum.CANCELLED))
...
/**
 * Finds the command to install OS packages. Prefers PackageKit pkcon/pkgcli
